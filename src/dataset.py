...
class SSSDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            # cv2.imwrite('./sem_debug.png', sem.astype(np.uint8)*255)
            sem = np.stack([~sem, sem]).astype(np.uint8)

            print(np.max(img), np.max(sem), np.max(ins))
