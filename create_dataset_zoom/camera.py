...
# def setCirclePathCamera(j, num):
    print('start setCirclePathCamera')
    clip_end = 999999
    ...
    #撮影画質設定(１あたり０．５ピクセル)
    bpy.context.scene.render.resolution_x = 2000    # 500pixel
    bpy.context.scene.render.resolution_y = 2000    # 500pixel
    
    #撮影開始
    # cs = [50, 50, 50, 50, 50]
    circle_scale = [40, 40, 40, 40, 40]
    circle_location = [0, 20, 40, 60, 80]
    # circle_location = [20, 20, 20, 20, 20]
    # camela_focus = [60., 50., 60., 70., 80.]
    camela_focus = [50., 50., 50., 50., 50.]
    # empty_location = [5, 5, 10, 15, 20]
    empty_location = [10, 15, 20, 25, 30]
    ...
    for i in range(5):
        circle_ob.scale = (circle_scale[i], circle_scale[i], circle_scale[i])
        circle_ob.location = (0, 0, circle_location[i])
        cam_ob.data.lens = camela_focus[i]
        empty_ob.location = (0, 0, empty_location[i])
